...
class TestGeneralLedgerReport(AccountTestInvoicingCommon):
    ...
    def test_general_ledger_account_range_onchange_string_domain(self):
        """GL wizard must use string code bounds (Odoo 19: int caused TypeError)."""
        company = self.env.user.company_id
        digit_accounts = self.env["account.account"].search(
            [("company_ids", "in", company.ids), ("code", "!=", False)]
        ).filtered(lambda a: a.code and a.code.isdigit())
        self.assertGreaterEqual(
            len(digit_accounts), 2, "Need at least two digit-code accounts"
        )
        codes_sorted = sorted(digit_accounts, key=lambda a: a.code)
        code_from = codes_sorted[0]
        code_to = codes_sorted[-1]
        wiz = self.env["general.ledger.report.wizard"].new(
            {
                "date_from": self.fy_date_start,
                "date_to": self.fy_date_end,
                "company_id": company.id,
        ...
        wiz.account_code_from = code_from
        wiz.account_code_to = code_to
        wiz.on_change_account_range()
        self.assertTrue(
            wiz.account_ids,
            "on_change_account_range should populate account_ids for a digit range",
        )
