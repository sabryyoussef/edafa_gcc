...
class TestOpenItems(AccountTestInvoicingCommon):
    ...
    def test_open_items_account_range_onchange_string_domain(self):
        """String code bounds must not raise TypeError (Odoo 19 RPC on onchange)."""
        company = self.env.user.company_id
        digit_reconcile = self.env["account.account"].search(
            [("company_ids", "in", company.ids), ("reconcile", "=", True)]
        ).filtered(lambda a: a.code and a.code.isdigit())
        self.assertGreaterEqual(
            len(digit_reconcile), 2, "Need two reconcilable digit-code accounts"
        )
        ordered = sorted(digit_reconcile, key=lambda a: a.code)
        # create() + initial account_ids: required field; onchange repopulates range.
        wiz = self.env["open.items.report.wizard"].create(
            {
                "date_at": Date.today(),
                "company_id": company.id,
                "account_ids": [(6, 0, [ordered[0].id])],
            }
        )
        wiz.account_code_from = ordered[0]
        wiz.account_code_to = ordered[-1]
        wiz.on_change_account_range()
        self.assertTrue(
            wiz.account_ids,
            "on_change_account_range should populate account_ids without RPC error",
        )
