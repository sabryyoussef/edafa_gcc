        ...
        for line in self.quantity_line:
            bill_line.append(
                (0, False, {'product_id': line.product_id.id,
                            ...
                            'cost_header_id': line.cost_header_id.id if line.cost_header_id else False,
...
class bill_quantity_line(models.Model):
    ...
    cost_header_id = fields.Many2one(
        'cost.header', string='Cost Header', )
    cost_code_id = fields.Many2one(
        'cost.code', string='Cost Code', 
        domain="[('cost_code_id', '=', cost_header_id)]")
    ...
    @api.onchange("cost_header_id")
    def _onchange_cost_header_id(self):
        """Clear cost_code when cost_header changes"""
        if self.cost_header_id:
            # Filter cost codes to only show those related to the selected header
            self.cost_code_id = False
        else:
            self.cost_code_id = False
